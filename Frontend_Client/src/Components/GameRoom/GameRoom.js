...
const GameRoom = () => {
    ...
    function drawSeparator() {
        for (let i = 0; i <= canvas.current.height; i += 20) {
            drawRectangle(getSeparator(canvas).x, getSeparator(canvas).y + i, getSeparator(canvas).width, getSeparator(canvas).height, getSeparator(canvas).color)
        }
    }

    const paint = () => {
        // drawRectangle(0, 0, canvas.current.width, canvas.current.height, "red");
        drawScore(getUser1(canvas).score, canvas.current.width / 4, canvas.current.height / 5);
        drawScore(getUser2(canvas).score, 3 * canvas.current.width / 4, canvas.current.height / 5);
        drawSeparator();
        drawRectangle(getUser1(canvas).x, getUser1(canvas).y, getUser1(canvas).width, getUser1(canvas).height, getUser1(canvas).color);
        drawRectangle(getUser2(canvas).x, getUser2(canvas).y, getUser2(canvas).width, getUser2(canvas).height, getUser2(canvas).color);
        drawCircle(getBall(canvas).x, getBall(canvas).y, getBall(canvas).radius, getBall(canvas).color);
    }
